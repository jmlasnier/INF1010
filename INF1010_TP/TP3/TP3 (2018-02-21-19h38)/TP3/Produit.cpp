...
Produit::Produit(Fournisseur& fournisseur,const string& nom, int reference, double prix,TypeProduit type) : //Liste d'initialisation ajouté
	...
	type_{type}
{
	fournisseur_.ajouterProduit(this);			//Utilisation de la méthode de la classe Fournisseur pour ajouter produit
}
...
{
	return fournisseur_;					//Retourne le fournisseur associé au produit
}
TypeProduit Produit::retournerType()  const
{
	return type_;							//Retounrne le type associé au produit
}
...
{
	is >> produit.nom_ >> produit.reference_ >> produit.prix_ >> produit.type_;
	return is;
}
